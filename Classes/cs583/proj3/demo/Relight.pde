...
void load(String path, String lightPath)
{
    ...
    for (int i = 0; i < mask.width; i++)
    {
        ...
        {
            if ( red(mask.get(i, j))/255.0 > 0 || green(mask.get(i, j))/255 > 0 || blue(mask.get(i, j))/255 > 0 )
            {                
                minX = min(minX, i);
                minY = min(minY, j);
                maxX = max(maxX, i);
...
DenseVector computeOneAlbedo(PImage[] images, DenseMatrix lights, int x, int y, DenseVector normal)
{
    ...
    for (int i = 0; i < NoI; i++)
    {
        color Color = images[i].get(x, y);
        IR.set(i, red(Color)/255.0);
        IG.set(i, green(Color)/255.0);
        IB.set(i, blue(Color)/255.0);
    }
    Sn.solve(IR, RouR);
    Sn.solve(IG, RouG);
    Sn.solve(IB, RouB);    
...
DenseVector computeOneNormal(PImage[] images,  DenseMatrix lights, int x, int y)
{
    ...
    for (int i = 0; i < NoI; i++)
    {       
        float IntensityPixel = 0.3*red(images[i].get(x, y))/255 + 0.59*green(images[i].get(x, y))/255 + 0.11*blue(images[i].get(x, y))/255;
